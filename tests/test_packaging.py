...
def test_numeralform_dependency_contract() -> None:
    ...
    assert "numeralform>=0.1.9,<0.2.0" in dependencies
...
def test_abbr2words_minimum_matches_structured_identity_contract() -> None:
    ...
    assert "abbr2words>=0.2.16,<0.3.0" in dependencies
...
def test_released_abbr2words_0213_contract() -> None:
    import abbr2words
    ...
    expander = Expander("en", registered_initialism_mode="spell")
    expander.add_many(
        (
    ...
    assert expander.expand_with_replacements("AAR AO AAA").replacements
