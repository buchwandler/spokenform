...
def test_french_replacements_have_exact_source_and_output_coordinates() -> None:
    ...
    assert list(result.source_replacements) == sorted(
        result.source_replacements, key=lambda item: (item.source_start, item.output_start)
    )
...
def test_french_structured_replacements_preserve_repeated_source_spans() -> None:
    ...
    assert [
        (item.start, item.end, item.text, item.language, item.rule) for item in replacements
    ] == [
...
def test_french_canonical_inventory_covers_base_and_extended_units() -> None:
    cases = {
        ...
        "2 l": "deux litres",
        "2 m": "deux mètres",
        "2 km/h": "deux kilomètres par heure",
        "2 m²": "deux mètres carrés",
        "2 m³": "deux mètres cubes",
    }
...
def test_french_dotted_duration_consumes_period_only_when_sentence_final() -> None:
    assert (
        prepare("45 min. puis 30 sec.", language="fr", use_spacy=False).spoken_text
        == "quarante-cinq minutes puis trente secondes."
    )
    assert (
        prepare("45 min. Puis", language="fr", use_spacy=False).spoken_text
        == "quarante-cinq minutes Puis"
    )
...
def test_french_protection_is_fail_closed_for_partial_quantity_and_literals() -> None:
    ...
    result = prepare(
        source, language="fr", use_spacy=False, protected_spans=[(partial, partial + 2)]
    )
    assert (
        result.spoken_text
        == "URL https://example.org/2kg email dev2@example.org v1.2.3 2 kg et trois kilogrammes"
    )
...
def test_french_ordinal_suffixes_do_not_corrupt_numbers_or_dates() -> None:
    result = prepare(
        "1er 1ère 1re 2e 2ème 2nd 2nde 3e 21e 14.05.2026", language="fr", use_spacy=False
    )
    assert (
        result.spoken_text
        == "premier première première deuxième deuxième second seconde troisième vingt et unième quatorze mai deux mille vingt-six"
    )
